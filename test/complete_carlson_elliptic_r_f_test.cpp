...
RC_GTEST_PROP(complete_carlson_elliptic_r_f_test, mathematical_properties, (double x)) {
    // RapidCheck generates random values for x
    RC_PRE(std::isfinite(x) && std::abs(x) < 100.0); // Precondition for valid domain
    
    double result = special_functions::complete_carlson_elliptic_r_f(x, 1.0, 2.0);
    ...
    // 3. TODO: Add specific mathematical properties for complete_carlson_elliptic_r_f:
    ...
    // RC_ASSERT(special_functions::complete_carlson_elliptic_r_f(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::complete_carlson_elliptic_r_f(-x) == special_functions::complete_carlson_elliptic_r_f(x)); // for even functions
    // RC_ASSERT(special_functions::complete_carlson_elliptic_r_f(-x) == -special_functions::complete_carlson_elliptic_r_f(x)); // for odd functions
}

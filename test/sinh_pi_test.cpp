...
RC_GTEST_PROP(sinh_pi_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for sinh_pi:
    ...
    // RC_ASSERT(special_functions::sinh_pi(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::sinh_pi(-x) == special_functions::sinh_pi(x)); // for even functions
    // RC_ASSERT(special_functions::sinh_pi(-x) == -special_functions::sinh_pi(x)); // for odd functions
}

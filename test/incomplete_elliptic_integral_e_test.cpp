...
RC_GTEST_PROP(incomplete_elliptic_integral_e_test, mathematical_properties, (double x)) {
    // RapidCheck generates random values for x
    RC_PRE(std::isfinite(x) && std::abs(x) < 100.0); // Precondition for valid domain
    
    double result = special_functions::incomplete_elliptic_integral_e(x, 1.0, 2.0);

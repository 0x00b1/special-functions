...
RC_GTEST_PROP(airy_bi_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for airy_bi:
    ...
    // RC_ASSERT(special_functions::airy_bi(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::airy_bi(-x) == special_functions::airy_bi(x)); // for even functions
    // RC_ASSERT(special_functions::airy_bi(-x) == -special_functions::airy_bi(x)); // for odd functions
}

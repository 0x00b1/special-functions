...
RC_GTEST_PROP(trigamma_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for trigamma:
    ...
    // RC_ASSERT(special_functions::trigamma(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::trigamma(-x) == special_functions::trigamma(x)); // for even functions
    // RC_ASSERT(special_functions::trigamma(-x) == -special_functions::trigamma(x)); // for odd functions
}

...
RC_GTEST_PROP(nome_q_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for nome_q:
    ...
    // RC_ASSERT(special_functions::nome_q(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::nome_q(-x) == special_functions::nome_q(x)); // for even functions
    // RC_ASSERT(special_functions::nome_q(-x) == -special_functions::nome_q(x)); // for odd functions
}

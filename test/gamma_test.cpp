...
RC_GTEST_PROP(gamma_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for gamma:
    ...
    // RC_ASSERT(special_functions::gamma(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::gamma(-x) == special_functions::gamma(x)); // for even functions
    // RC_ASSERT(special_functions::gamma(-x) == -special_functions::gamma(x)); // for odd functions
}

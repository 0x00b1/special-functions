...
RC_GTEST_PROP(owens_t_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for owens_t:
    ...
    // RC_ASSERT(special_functions::owens_t(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::owens_t(-x) == special_functions::owens_t(x)); // for even functions
    // RC_ASSERT(special_functions::owens_t(-x) == -special_functions::owens_t(x)); // for odd functions
}

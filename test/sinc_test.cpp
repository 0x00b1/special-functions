...
RC_GTEST_PROP(sinc_test, mathematical_properties, (double x)) {
    ...
    // 3. TODO: Add specific mathematical properties for sinc:
    ...
    // RC_ASSERT(special_functions::sinc(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::sinc(-x) == special_functions::sinc(x)); // for even functions
    // RC_ASSERT(special_functions::sinc(-x) == -special_functions::sinc(x)); // for odd functions
}

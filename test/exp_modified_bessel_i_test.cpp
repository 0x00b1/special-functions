...
RC_GTEST_PROP(exp_modified_bessel_i_test, mathematical_properties, (double x)) {
    // RapidCheck generates random values for x
    RC_PRE(std::isfinite(x) && std::abs(x) < 100.0); // Precondition for valid domain
    
    double result = special_functions::exp_modified_bessel_i(x, 1.0, 2.0);
    ...
    // 3. TODO: Add specific mathematical properties for exp_modified_bessel_i:
    ...
    // RC_ASSERT(special_functions::exp_modified_bessel_i(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::exp_modified_bessel_i(-x) == special_functions::exp_modified_bessel_i(x)); // for even functions
    // RC_ASSERT(special_functions::exp_modified_bessel_i(-x) == -special_functions::exp_modified_bessel_i(x)); // for odd functions
}

...
RC_GTEST_PROP(zernike_polynomial_z_test, mathematical_properties, (double x)) {
    // RapidCheck generates random values for x
    RC_PRE(std::isfinite(x) && std::abs(x) < 100.0); // Precondition for valid domain
    
    double result = special_functions::zernike_polynomial_z(x, 1.0, 2.0);
    ...
    // 3. TODO: Add specific mathematical properties for zernike_polynomial_z:
    ...
    // RC_ASSERT(special_functions::zernike_polynomial_z(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::zernike_polynomial_z(-x) == special_functions::zernike_polynomial_z(x)); // for even functions
    // RC_ASSERT(special_functions::zernike_polynomial_z(-x) == -special_functions::zernike_polynomial_z(x)); // for odd functions
}

...
RC_GTEST_PROP(incomplete_beta_test, mathematical_properties, (double x)) {
    // RapidCheck generates random values for x
    RC_PRE(std::isfinite(x) && std::abs(x) < 100.0); // Precondition for valid domain
    
    double result = special_functions::incomplete_beta(x);
    ...
    // 3. TODO: Add specific mathematical properties for incomplete_beta:
    ...
    // RC_ASSERT(special_functions::incomplete_beta(0.0) == expected_value_at_zero);
    // RC_ASSERT(special_functions::incomplete_beta(-x) == special_functions::incomplete_beta(x)); // for even functions
    // RC_ASSERT(special_functions::incomplete_beta(-x) == -special_functions::incomplete_beta(x)); // for odd functions
}

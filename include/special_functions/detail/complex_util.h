#ifndef SPECIAL_FUNCTIONS_DETAIL_COMPLEX_UTIL_H
#define SPECIAL_FUNCTIONS_DETAIL_COMPLEX_UTIL_H
...
//
    ...
    template<typename Tp>
    bool
    fp_is_real(const std::complex<Tp> &w, const Tp mul = Tp{1}) { return fp_is_zero(std::imag(w), mul); }

    // Specialize for real numbers.
    template<typename Tp>
    bool
    fp_is_real(const Tp) { return true; }
    ...
    template<typename Tp>
    bool
    fp_is_imag(const std::complex<Tp> &w, const Tp mul = Tp{1}) { return fp_is_zero(std::real(w), mul); }

    // Specialize for real numbers.
    template<typename Tp>
    bool
    fp_is_imag(const Tp) { return false; }
    ...
    template<typename T>
    struct fp_promote_help<std::complex<T>, false> {
    private:
        using vtype = typename std::complex<T>::value_type;
    public:
    ...
    template<typename Tp>
    struct is_complex<const Tp> : public is_complex<Tp> {
    };

    template<typename Tp>
    struct is_complex<volatile Tp> : public is_complex<Tp> {
    };

    template<typename Tp>
    struct is_complex<const volatile Tp> : public is_complex<Tp> {
    };
...
#endif // SPECIAL_FUNCTIONS_DETAIL_COMPLEX_UTIL_H
